...
def solution(alp, cop, problems):
    ...
    for alp_score in range(alp, max_alp + 1):
        for cop_score in range(cop, max_cop + 1):
            curr_time = dp[alp_score][cop_score]
            if alp_score + 1 <= max_alp:  # +1 alp
                dp[alp_score + 1][cop_score] = min(dp[alp_score + 1][cop_score], curr_time + 1)
            if cop_score + 1 <= max_cop:  # +1 cop
                dp[alp_score][cop_score + 1] = min(dp[alp_score][cop_score + 1], curr_time + 1)
            ...
            for problem in problems:
                ...
                if alp_score >= nalp and cop_score >= ncop:
                    ...
                    dp[ni][nj] = min(dp[ni][nj], curr_time + time)
